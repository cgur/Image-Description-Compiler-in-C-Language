...
Boolean saveImage ( Image image , char * fileName )
{
  ...
  if( fprintf( fp, "%d %d\n", image.width, image.height)<= 0) {return FALSE;}
  if( fprintf( fp, "255\n"                             )<= 0) {return FALSE;}
...
)
{
  ...
  double angle = (endAngleDegrees - startAngleDegrees) / numberOfSegments ;

  startAngleDegrees = (startAngleDegrees * PI) / 180; 

  endAngleDegrees   = (endAngleDegrees   * PI) / 180; 

  angle = (angle * PI) / 180;

  curr.row = (int) (radius * cos(PI - (angle*i + startAngleDegrees))+ center.row);
  curr.col = (int) (radius * sin(PI - (angle*i + startAngleDegrees))+ center.col);
  ...
  if (drawLine(image, prev, curr, color)) 
    {
      prev = curr;
      i++;
    }  
...
{
  Point TopLeft   , TopRight;    /* Top Line   : from Top Left   , to Top Right      */
  Point BottomLeft, BottomRight; /* Bottom Line: from Bottom Left, to Bottom Right   */

  TopLeft = topLeft                          ;

  TopRight.row    = TopLeft.row              ;
  TopRight.col    = TopLeft.col + (width-1 ) ;

  BottomLeft.row  = TopLeft.row + (height-1) ; 
  BottomLeft.col  = TopLeft.col              ;

  BottomRight.row = BottomLeft.row           ;
  BottomRight.col = TopRight.col             ;

  if((drawLine(image, TopLeft,    TopRight,    color)) &&  /* Top Line is created.    */
     (drawLine(image, BottomLeft, BottomRight, color)) &&  /* Bottom Line is created. */
     (drawLine(image, TopLeft,    BottomLeft,  color)) &&  /* Left Line is created.   */
     (drawLine(image, TopRight,   BottomRight, color)))    /* Right Line is created.  */
     {return TRUE;} 
...
Boolean drawRegularConvexPolygon ( Image image , Point center , unsigned short int radius , unsigned short int numberOfSides , Color color )
{
  ...
  curr.row = (int) (radius * cos(PI - (((2*PI*i))/(numberOfSides)))+ center.row);
  curr.col = (int) (radius * sin(PI - (((2*PI*i))/(numberOfSides)))+ center.col);
